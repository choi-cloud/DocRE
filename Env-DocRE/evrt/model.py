...
class DocREModel(nn.Module):
    ...
    def get_hrt(self, sequence_output, attention, entity_pos, hts):
        offset = (
            1 if self.config.transformer_type in ["bert", "roberta", "deberta"] else 0
        )
        ...
        for i in range(len(entity_pos)):
            ...
            h_att = torch.index_select(
                entity_atts, 0, ht_i[:, 0]
            )  # [n_e*(n_e-1), h, seq_len]
            t_att = torch.index_select(
                entity_atts, 0, ht_i[:, 1]
            )  # [n_e*(n_e-1), h, seq_len]
            ht_att = (h_att * t_att).mean(1)  # [n_e*(n_e-1), seq_len]
            ht_att = ht_att / (
                ht_att.sum(1, keepdim=True) + 1e-5
            )  # [n_e*(n_e-1), seq_len]
    ...
    def forward(
        ...
        output = (
            self.loss_fnt.get_label(logits, num_labels=self.num_labels),
            logits,
            bl,
        )  # [b*n_e*(n_e-1), num_classes]
