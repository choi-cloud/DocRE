    ...
    def finetune(features, optimizer, num_epoch, num_steps):
        ...
        if args.evrt:
            ...
            if os.path.exists(os.path.join(args.data_dir, args.evrt_file + suffix)):
                crt_features = torch.load(
                    os.path.join(args.data_dir, args.evrt_file + suffix)
                )
            else:
                crt_file = os.path.join(args.data_dir, args.evrt_file)
                crt_features = read_docred(
                    crt_file, rel2id, tokenizer, max_seq_length=args.max_seq_length
                )
                torch.save(
                    crt_features, os.path.join(args.data_dir, args.evrt_file + suffix)
                )
            crt_titles_to_features = {
                feature["title"]: feature for feature in crt_features
            }
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.evrt:
                    crt_batch = collate_fn(
                        [crt_titles_to_features[title] for title in batch[5]]
                    )
                    crt_inputs = {
                        "input_ids": crt_batch[0].to(args.device),
                ...
                if step == len(train_dataloader) - 1 or (
                    ...
                    dev_score, dev_output = evaluate(
                        args, model, dev_features, rel2id, id2rel, tag="dev"
                    )
...
def evaluate(args, model, features, rel2id, id2rel, tag="dev"):
    ...
    if tag == "dev":
        result = official_evaluate_benchmark(
            ans, args.data_dir, args.train_file, args.dev_file, rel2id
        )
    else:
        result = official_evaluate_benchmark(
            ans, args.data_dir, args.train_file, args.test_file, rel2id
        )
    output = {
...
def main():
    ...
    parser.add_argument(
        "--evrt", action="store_true", help="Enable entity variance robust training."
    )
    parser.add_argument(
        "--pcrloss_weight", default=1.0, type=float, help="Weight of PcrLoss."
    )
    parser.add_argument(
        "--rcrloss_weight", default=1.0, type=float, help="Weight of RcrLoss."
    )
    parser.add_argument(
        "--num_train_epochs",
        default=8.0,
    ...
    if os.path.exists(os.path.join(args.data_dir, args.train_file + suffix)):
        train_features = torch.load(
            os.path.join(args.data_dir, args.train_file + suffix)
        )
        print("Loaded train features")
    else:
        train_file = os.path.join(args.data_dir, args.train_file)
        train_features = read(
            train_file, rel2id, tokenizer, max_seq_length=args.max_seq_length
        )
        torch.save(
