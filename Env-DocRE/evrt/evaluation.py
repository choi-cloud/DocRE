...
def to_official_by_doc(preds, features, id2rel):
    h_idx, t_idx, title = [], [], []
    res = []
...
def official_evaluate(tmp, path, train_file, dev_file):
    ...
    for x in truth:
        ...
        for label in x["labels"]:
            ...
            tot_evidences += len(set(label["evidence"]))
...
def official_evaluate_benchmark(tmp, path, train_file, dev_file, rel2id):
    """
    Adapted from the official evaluation code
    """
    freq_keys = {
        ...
        for label in x["labels"]:
            ...
            if (
                findSmallestDifference(
                    h_sent_set, t_sent_set, len(h_sent_set), len(t_sent_set)
                )
                == 0
            ):
                std_intra[(title, r, h_idx, t_idx)] = set(label["evidence"])
            if 1 <= findSmallestDifference(
                h_sent_set, t_sent_set, len(h_sent_set), len(t_sent_set)
            ):
                std_inter[(title, r, h_idx, t_idx)] = set(label["evidence"])
            if 5 < findSmallestDifference(
                h_sent_set, t_sent_set, len(h_sent_set), len(t_sent_set)
            ):
    ...
    submission_answer_long_tail = [
        x for x in submission_answer if x["r"] in long_tail_keys
    ]
    ...
    if re_p_long_tail + re_r_long_tail == 0:
        ...
        re_f1_long_tail = (
            2.0 * re_p_long_tail * re_r_long_tail / (re_p_long_tail + re_r_long_tail)
        )
